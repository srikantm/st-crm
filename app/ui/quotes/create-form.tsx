...
import { CreateInvoiceForm, CurriencyField, CustomerField, InvoiceItem, LeadForm, PackageField, TaxField } from '@/app/lib/definitions';
...
import { createQuoteForm } from '@/app/lib/actions';
import { Col, Divider, Row, Form as Frm, Input, InputNumber, Button as Btn } from 'antd';
...
export default function Form({ customers, currencies, taxes, packages, leads }: {
  customers: CustomerField[],
  currencies: CurriencyField[], taxes: TaxField[], packages: PackageField[], leads: LeadForm[]
}) {
  ...
  const [formData, setFormData] = useState<CreateInvoiceForm>({
    ...
    customerId: "",
    leadId: "",
    invoiceNumber: `QT${(new Date).valueOf().toString()}`,
    currencyId: "",
    currencySymbol: "",
    invoiceStatus: "",
    invoiceDate: "",
  ...
  const handleProductChange = (e: React.ChangeEvent<HTMLSelectElement>, index: number) => {
    const { name, value } = e.target;
    if (name === 'productId') {
      // const packageRates = fetchPackageRates(value); 
  ...
  const handleDateChange = (date: Dayjs, name: string) => {
    const formatedDated = date.format('YYYY-MM-DD');
    if (name === 'invoiceDate') {
  ...
  return (
    ...
    <>
      <div className="rounded-md bg-gray-50 p-4 md:p-6">
        <div className="flex flex-wrap">
          ...
          <div className="p-3 mb-5 basis-1/4">
            ...
            {/* {formData.customerId === '' && <label htmlFor="error" className={`${font.className} mb-2 block text-xs text-red-500`}>Please Select Customer</label>} */}
          ...
          <div className="p-3 mb-4 basis-1/4">
            ...
            {formData.invoiceStatus === '' && <label htmlFor="error" className={`${font.className} mb-2 block text-xs text-red-500`}>Please select Status </label>}
          </div>
        ...
        <div className="flex flex-wrap">
          ...
          <div className="p-3 mb-4 basis-1/4">
            ...
            <div className="relative">
              <DatePicker style={{ width: '100%' }} format={'YYYY-MM-DD'} onChange={(e) => handleDateChange(e, 'invoiceDate')} />
            </div>
            {formData.invoiceDate === '' && <label htmlFor="error" className={`${font.className} mb-2 block text-xs text-red-500`}>Please select Quote Date </label>}
          </div>
          <div className="p-3 mb-4 basis-1/4">
            ...
            <div className="relative">
              <DatePicker style={{ width: '100%' }} format={'YYYY-MM-DD'} onChange={(e) => handleDateChange(e, 'invoiceExpiryDate')} />
            </div>
            {formData.invoiceExpiryDate === '' && <label htmlFor="error" className={`${font.className} mb-2 block text-xs text-red-500`}>Please select Expiry Date </label>}
          </div>
        ...
        <div >
          ...
          <Row gutter={[12, 12]} style={{ position: 'relative', paddingTop: 15 }}>
            ...
            <Col className="gutter-row" span={4}>
              ...
              {formData.invoiceTaxId === '' && <label htmlFor="error" className={`${font.className} mb-2 block text-xs text-red-500`}>Please Select Tax</label>}
            </Col>
            <Col className="gutter-row" span={4}>
              <div className="relative">
                <InputNumber
      ...
      <div className="mt-6 flex justify-end gap-4">
        ...
        <Button onClick={saveQuote}>Create Quote</Button>
      </div>
    </>
  );
}
